...
EXT_CODE = ['c', 'py', 'java', 'html', 'css', 'js']
EXT_INSTL = ['dmg', 'exe', 'iso']

#Create directories to store the files
BASE_PATH = os.path.expanduser('~')
DEST_DIRS = ['Music', 'Movies', 'Pictures', 'Documents', 'Applications', 'Code', 'Other']
...
for f_ext, f_list in files_mapping.items():
    ...
    if f_ext in EXT_INSTL:
        ...
        for file in f_list:
            os.rename(os.path.join(DOWNLOADS_PATH, file), os.path.join(BASE_PATH, 'Code', file))
